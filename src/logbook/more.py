...
class TwitterHandler(Handler, StringFormatterHandlerMixin):
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "The python-oauth2 library is required for "
                "the TwitterHandler."
            )
...
class SlackHandler(Handler, StringFormatterHandlerMixin):
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "The Python Slack SDK library is required for "
                "the SlackHandler."
            )
    ...
    def emit(self, record):
        self.slack.chat_postMessage(
            channel=self.channel, text=self.format(record)
        )
...
class ExternalApplicationHandler(Handler):
    ...
    def emit(self, record):
        ...
        if self._stdin_format is not None:
            stdin_data = self._stdin_format.format(record=record).encode(
                self.encoding
            )
...
class RiemannHandler(Handler):
    ...
    def __init__(
        ...
        :param message_type: selects transport. Currently available 'tcp' and
            'udp'
