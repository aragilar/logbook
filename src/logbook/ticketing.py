...
class SQLAlchemyBackend(BackendBase):
    ...
    def create_tables(self):
        ...
        def table(name, *args, **kwargs):
            return db.Table(
                self.table_prefix + name, self.metadata, *args, **kwargs
            )
...
class MongoDBBackend(BackendBase):
    ...
    def setup_backend(self):
        ...
        database.tickets.ensure_index(
            [("record_hash", ASCENDING)], unique=True
        )
        database.tickets.ensure_index(
            [("solved", ASCENDING), ("level", ASCENDING)]
        )
    ...
    def solve_ticket(self, ticket_id):
        """Marks a ticket as solved."""
        self.database.tickets.update(
            {"_id": self._oid(ticket_id)}, {"solved": True}
        )
    ...
    def get_occurrences(self, ticket, order_by="-time", limit=50, offset=0):
        ...
        occurrences = (
            self._order(
                collection.find({"ticket_id": self._oid(ticket)}), order_by
            )
            .limit(limit)
            .skip(offset)
        )
