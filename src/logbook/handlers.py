...
class NullHandler(Handler):
    ...
    def __init__(self, level=NOTSET, filter=None):
        super(NullHandler, self).__init__(
            level=level, filter=filter, bubble=False
        )
    ...
    def check_delivery(self, record):
        ...
        try:
            ...
            if (
                ...
                del cache_items[
                    : int(self._record_limits) * self.record_cache_prune
                ]
...
class GZIPCompressionHandler(FileHandler):
    def __init__(
        ...
        super(GZIPCompressionHandler, self).__init__(
            filename,
            mode="wb",
            encoding=encoding,
            level=level,
...
class BrotliCompressionHandler(FileHandler):
    def __init__(
        ...
        super(BrotliCompressionHandler, self).__init__(
            filename,
            mode="wb",
            encoding=encoding,
            level=level,
        ...
        try:
            ...
            raise RuntimeError(
                "The brotli library is required for "
...
class MonitoringFileHandler(FileHandler):
    ...
    def __init__(
        ...
        if os.name == "nt":
            raise RuntimeError(
                "MonitoringFileHandler " "does not support Windows"
            )
        self._query_fd()
...
class StderrHandler(StreamHandler):
    ...
    def __init__(
        self, level=NOTSET, format_string=None, filter=None, bubble=False
    ):
        StreamHandler.__init__(
...
class RotatingFileHandler(FileHandler):
    ...
    def __init__(
        ...
        assert backup_count > 0, (
            "at least one backup file has to be " "specified"
        )
...
class TimedRotatingFileHandler(FileHandler):
    ...
    def perform_rollover(self, new_timestamp):
        ...
        if not self.timed_filename_for_current and os.path.exists(
            self._filename
        ):
            filename = self.generate_timed_filename(self._timestamp)
...
class TestHandler(Handler, StringFormatterHandlerMixin):
    ...
    def formatted_records(self):
        ...
        if len(self._formatted_record_cache) != len(self.records) or any(
            r1 != r2
            for r1, r2 in zip(self.records, self._formatted_record_cache)
        ):
...
class MailHandler(Handler, StringFormatterHandlerMixin, LimitingHandlerMixin):
    ...
    def get_connection(self):
        ...
        # Previously, self.secure was passed as con.starttls(*self.secure).
        # This meant that starttls couldn't be used without a keyfile and
        # certfile unless an empty tuple was passed. See issue #94.
        #
...
class SyslogHandler(Handler, StringFormatterHandlerMixin):
    ...
    def wrap_segments(self, record, before):
        ...
        return (
            before + segment + self.record_delimiter for segment in segments
        )
...
class NTEventLogHandler(Handler, StringFormatterHandlerMixin):
    ...
    def __init__(
        ...
        self._welu.AddSourceToRegistry(
            self.application_name, dllname, log_type
        )
    ...
    def unregister_logger(self):
        ...
        """
        self._welu.RemoveSourceFromRegistry(
            self.application_name, self.log_type
        )
    ...
    def enqueue(self, record):
        ...
        if self._action_triggered:
            ...
            if self._buffer_full:
                ...
                self.buffer_size
                and len(self.buffered_records) >= self.buffer_size
            ):
...
class GroupHandler(WrapperHandler):
    ...
    _direct_attrs = frozenset(
        ["handler", "pull_information", "buffered_records"]
    )
