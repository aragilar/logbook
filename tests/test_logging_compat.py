...
def test_basic_compat(request, set_root_logger_level):
    ...
    request.addfinalizer(
        functools.partial(logging.root.setLevel, logging.root.level)
    )
    ...
    with logbook.TestHandler(bubble=True) as handler:
        with capturing_stderr_context() as captured:
            with redirected_logging(set_root_logger_level):
                ...
                logger.error(
                    "This is a %(what)s %(where)s",
                    {"what": "mapping", "where": "test"},
                )
        assert (
            "WARNING: %s: This is from the old system" % name
        ) in captured.getvalue()
        assert (
            "ERROR: %s: This is a mapping test" % name
        ) in captured.getvalue()
...
def test_redirect_logbook_respect_specific_configuration():
    ...
    config = {
        ...
        "loggers": {
            "module_2": {"handlers": ["console_2"], "propagate": False}
        },
    }
    logger = logging.getLogger()
...
def test_warning_redirections():
    ...
    with logbook.TestHandler() as handler:
        ...
        try:
            ...
            warn(
                RuntimeWarning(
                    "Testing" + str(next(test_warning_redirections_i))
                )
            )
        finally:
            redirector.end()
