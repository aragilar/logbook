...
def test_rotating_file_handler(logfile, activation_strategy, logger):
    ...
    files = [
        x
        for x in os.listdir(os.path.dirname(logfile))
...
def test_timed_rotating_file_handler(
    ...
    assert (
        files
        == ["trot-2010-01-0{0}.log".format(i) for i in range(5, 9)][
            -backup_count:
        ]
    )
...
def test_timed_rotating_file_handler__rollover_format(
    ...
    assert (
        files
        == ["trot.log.2010-01-0{0}".format(i) for i in range(5, 9)][
            -backup_count:
        ]
    )
...
def test_compression_file_handler(
    logfile, activation_strategy, logger, use_gzip
):
    handler = (
